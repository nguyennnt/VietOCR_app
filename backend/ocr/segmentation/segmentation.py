...
def segment(input_path, output_path):
    ...
    # find contours
    ctrs = cv2.findContours(img_dilation.copy(),
    ...
    max_width, max_height = image[y:y+h, x:x +
                                  w].shape[0], image[y:y+h, x:x+w].shape[1]
    valid_img = 0
    ...
    for i, ctr in enumerate(sorted_ctrs):
        ...
        if(width >= max_width*0.1 and height >= max_height*0.7):
            # show ROI
            cv2.imwrite(
                '{0}/img_transformer{1}.jpg'.format(output_path, valid_img), roi)
            valid_img += 1
